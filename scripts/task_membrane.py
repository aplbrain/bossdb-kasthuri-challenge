...
class AugmentBoth:
    """
    A class to perform data augmentation on both image and mask simultaneously.
    
    Attributes:
        p_flip (float): Probability of performing a horizontal flip on the image and mask.
            Default is 0.5.
        max_rotation (int): Maximum angle (in degrees) for random rotation. The rotation angle will be
            randomly chosen between -max_rotation and max_rotation. Default is 10.
    
    Methods:
        __call__(image, mask): Applies the defined augmentations on the input image and mask.
    """
    
    def __init__(self, p_flip=0.5, max_rotation=10):
        """
        Initializes the AugmentBoth class with the given parameters.
        
        Args:
            p_flip (float, optional): Probability of performing a horizontal flip on the image and mask.
                Default is 0.5.
            max_rotation (int, optional): Maximum angle (in degrees) for random rotation. Default is 10.
        """
        self.p_flip = p_flip
    ...
    def __call__(self, image, mask):
        """
        Applies the defined augmentations on the input image and mask.
        
        Args:
            image (torch.Tensor): The input image tensor to be augmented.
            mask (torch.Tensor): The input mask tensor to be augmented.
            
        Returns:
            tuple[torch.Tensor, torch.Tensor]: A tuple containing the augmented image and mask.
        """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', default="kasthuri/taskconfig/membrane_task.json",
